...
for i = 1 : size(mats,2)
  mat = strjoin(mats(i));
  [req,pot,add,F,c] = precol2('D2RestrictedColumnsNonReq','Nat','Nat','10','2',mat,-1);
  [req1,pot1,add1,F1,c1] = precol2('SBSchemeCombinedVertexCoverColoringRestrictedNonReq','Nat','Nat','10','2',mat,-1);
  Fs= [Fs nnz(F)];
  pots=[pots nnz(pot)];
  adds = [adds nnz(add)];
  cols = [cols c];
