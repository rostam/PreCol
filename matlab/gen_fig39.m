...
for i = 1 : 1: 80
  ...
  [req2,pot2,add2,F2,c2] = precol2('D2RestrictedColumnsNonReqBalanced','Nat','Nat',num2str(i),'2','nos3.mtx',6);
  [req3,pot3,add3,F3,c3] = precol2('D2RestrictedColumnsNonReqBalanced','Nat','Nat',num2str(i),'2','nos3.mtx',10);
  Fs= [Fs nnz(F)];
  pots=[pots nnz(pot)];
  adds = [adds nnz(add)];
  cols = [cols c];
