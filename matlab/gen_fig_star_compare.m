...
%   i
%   els = [els i];
%   [req,pot,add,F,c] = precol2('SBSchemeCombinedVertexCoverColoringRestricted','Nat','Nat',num2str(i),'2','crystm01.mtx',-1);
%   [req1,pot1,add1,F1,c1] = precol2('SBSchemeCombinedVertexCoverColoringRestrictedNonReq','Nat','Nat',num2str(i),'2','crystm01.mtx',-1);
%   Fs= [Fs nnz(F)];
%   pots=[pots nnz(pot)];
%   adds = [adds nnz(add)];
%   cols = [cols c];
