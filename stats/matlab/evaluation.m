...
for i=10 : 20 :  30
    i
    els = [els i];
    [req,pot,add,F] = precol2('LFO','Nat',int2str(i),'10','ex33.mtx',5);
    Fs= [Fs nnz(F)];
    pots=[pots nnz(pot)];
    adds = [adds nnz(add)];
    [req1,pot1,add1,F1] = precol2('LFO','Min',int2str(i),'10','ex33.mtx',5);
    Fs1= [Fs1 nnz(F1)];
    pots1=[pots1 nnz(pot1)];
    adds1 = [adds1 nnz(add1)];
    [req2,pot2,add2,F2] = precol2('LFO','Metis',int2str(i),'10','ex33.mtx',5);
    Fs2= [Fs2 nnz(F2)];
