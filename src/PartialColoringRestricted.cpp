 ...
 */
int PartialColoringRestricted(Graph& G_b) {
    ...
    //Initialize colors
    for (graph_traits<Graph>::vertex_iterator v = vertices(G_b).first; v != vertices(G_b).second; ++v) {
        put(color, *v, 0);
    }
    ...
    for (graph_traits<Graph>::vertex_iterator v = vertices(G_b).first; v != vertices(G_b).second; ++v) {
        ...
        if (IncidentToReqEdge(G_b, *v)) {
            ...
            N_2 = neighbors::restricted_neighbors(G_b, *v);
