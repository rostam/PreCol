...
for i = 1 : 1: 70
  i
  els = [els i];
  [req,pot,add,F,c] = precol2('D2RestrictedColumns','LFO','Nat',num2str(i),'2','ex33.mtx',-1);
  [req1,pot1,add1,F1,c1] = precol2('D2RestrictedColumnsNonReq','LFO','Nat',num2str(i),'2','ex33.mtx',-1);
  [req2,pot2,add2,F2,c2] = precol2('D2RestrictedColumnsNonReqBalanced','LFO','Nat',num2str(i),'2','ex33.mtx',-1);
  [req3,pot3,add3,F3,c3] = precol2('D2RestrictedColumnsNonReqBalanced','LFO','Nat',num2str(i),'2','ex33.mtx',alpha);
  Fs= [Fs nnz(F)];
  pots=[pots nnz(pot)];
  adds = [adds nnz(add)];
  cols = [cols c];
...
h=legend('Algorithm 3.2',...
         'Algorithm 3.4',...
         ['Algorithm 3.5 with \alpha=10',alpha]);
   set(h,'Location','Best',...
       'FontSize',22);
